...
class Security {
	static async injectDB(conn) {
		security = await conn.db("Assignment").collection("security")
	}
//register security
	...
	static async login(username, userpassword) {
		...
		const user = await security.findOne({$or: [{"name" : username}, {"password" : userpassword}]})
		.then(async user =>{
		...
		return security.findOne({
		  ...
		  if (user){
			if (user.password != userpassword){
			  ...
			  await security.deleteOne({"name" : username})
